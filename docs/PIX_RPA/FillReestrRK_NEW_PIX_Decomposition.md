...
Референсное разбиение метода `FillReestrRK_NEW` (файл `Program.cs`) на шаги для PIX RPA **без DTO и без конструкторов**. Исходная логика остаётся в коде, здесь — инструкция, как перенести в цепочку активностей «Вызов C#», опираясь только на переменные процесса.
